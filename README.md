...
5. После первого деплоя один раз выполните `npm run seed` (Railway →
   ...
   товарами. Дальше товары можно добавлять напрямую через
   `backend/src/models/products.js` → `createProduct` или руками в БД.
...
- (опционально) `/setcommands` — добавить `orders - Все заказы` для
  удобства.
